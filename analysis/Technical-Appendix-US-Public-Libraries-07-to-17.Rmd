...
PLS$LOCALE_NAME[PLS$LOCALE == 11] <- "City, Large"
PLS$LOCALE_NAME[PLS$LOCALE == 12] <- "City, Mid-Size"
PLS$LOCALE_NAME[PLS$LOCALE == 13] <- "City, Small"
PLS$LOCALE_NAME[PLS$LOCALE == 21] <- "Suburb, Large"
PLS$LOCALE_NAME[PLS$LOCALE == 22] <- "Suburb, Mid-Size"
PLS$LOCALE_NAME[PLS$LOCALE == 23] <- "Suburb, Small"
PLS$LOCALE_NAME[PLS$LOCALE == 31] <- "Town, Fringe"
PLS$LOCALE_NAME[PLS$LOCALE == 32] <- "Town, Distant"
PLS$LOCALE_NAME[PLS$LOCALE == 33] <- "Town, Remote"
PLS$LOCALE_NAME[PLS$LOCALE == 41] <- "Rural, Fringe"
PLS$LOCALE_NAME[PLS$LOCALE == 42] <- "Rural, Distant"
PLS$LOCALE_NAME[PLS$LOCALE == 43] <- "Rural, Remote"
# Convert LOCALE_NAME to factor and remove LOCALE
PLS <- mutate(PLS, LOCALE_NAME = as.factor(LOCALE_NAME)) %>% 
  select(-LOCALE)
...
PLS_FINAL <- select(PLS, ID, SRVY_YEAR, FSCSKEY, LIBNAME, ADDRESS, CITY, STABR, ZIP, CNTY, OBEREG, LEGAL_BASIS, LOCALE_NAME, CENTLIB, BRANLIB, BKMOB, LIBRARIA, LOCGVT, STGVT, FEDGVT, OTHINCM, STAFFEXP, PRMATEXP, ELMATEXP, OTHMATEX, TOTEXPCO, OTHOPEXP, TOTOPEXP, BKVOL, EBOOK, AUDIO_PH, AUDIO_DL, VIDEO_PH, VIDEO_DL, ELECCOLL, VISITS, REGBOR, TOTCIR, KIDCIRCL, ELMATCIR, PHYSCIR, LOANTO, LOANFM, TOTPRO, KIDPRO, YAPRO, TOTATTEN, KIDATTEN, YAATTEN, GPTERMS, PITUSR, WIFISESS, LONGITUD, LATITUDE)
```
...
- Create revenue and/or expenditure categories for easier groupings

Add new variables:
```{r}
# Add new variables to df-----------------------------------------------------------------------------
...
write.csv(PLS_FINAL, file = "Public_Library_Survey_08_to_17.csv")
```
